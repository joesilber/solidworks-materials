...
## GitHub clients
There are numerous graphical git clients. A common one is [GitHub Desktop](https://desktop.github.com/download/) on Windows or Mac. [Visual Studio Code](https://code.visualstudio.com/) works nicely, too. If you find a different client you like better, let the rest of us know.

Side note: as of this writing (2026-03-16) I found that both GitHub web interface and GitHub Desktop did not handle diffs well on the UTF-16 encoded `sldmat` files that Solidworks produces. The GitHub tools treated these files like binary, preventing line-by-line diffs. The solution was to add a `.gitattributes` file including the line
```
*.sldmat text working-tree-encoding=UTF-16LE-BOM eol=crlf
```
Commit history of `sldmat` files prior to this change will still be internally inscrutable on GitHub (though the diffs are viewable in Visual Studio Code). But after this they should all work on GitHub and GitHub Desktop ok.
